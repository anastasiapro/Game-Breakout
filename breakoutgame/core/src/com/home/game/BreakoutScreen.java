...
import com.home.game.BallActor;
import com.home.game.BrickActor;
import com.home.game.PaddleActor;
import com.home.game.BreakoutContactListener;
import com.home.game.Contdead;
import com.home.game.Wall;
...
public class BreakoutScreen implements Screen {
	...
	public BreakoutScreen(final BreakoutGame game) {
		...
		Color[] brickColors = new Color[2];
		brickColors[0] = new Color(240 / 255f, 209 / 255f, 40 / 255f, 1f);
		brickColors[1] = new Color(102 / 255f, 213 / 255f, 110 / 255f, 1f);
		for (int y = 0; y < 2; y++) {
			for (int x = 0; x < 12; x++) {
	...
	private void deleteDeadBodies() {
		...
		while (bodies.size > 0) {
			node = bodies.pop();
			Body oBj = node;
			if (((Contdead) oBj.getUserData()).isDying()) {
